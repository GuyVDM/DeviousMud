...
struct ImPool
{
    ...
    void        RemoveLayerEntity(ImGuiID key, const T* p)     { RemoveLayerEntity(key, GetIndex(p)); }
    void        RemoveLayerEntity(ImGuiID key, ImPoolIdx idx)  { Buf[idx].~T(); *(int*)&Buf[idx] = FreeIdx; FreeIdx = idx; Map.SetInt(key, -1); AliveCount--; }
